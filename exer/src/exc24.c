...
{
	Knoten dummy = { 0, NULL };
	Knoten *prev = &dummy, *curr = kopf;

	while (curr) {
		if (curr->data == x) {
			--anzahl;
			Knoten *next = curr->next;
			free(curr);
			prev->next = next;
			printf("Knoten mit dem Wert %d wurde geloescht.\n", x);
			return;
		}
	...
	printf("Der Wert %d kommt in der Liste nicht vor.\n", x);
}
...
int main() {
	...
	while (weiter) {
		...
		printf("6. Loesche ersten Knoten mit Wert\n");
		printf("7. Ende\n\n");
		printf("Auswahl: ");
		scanf("%d", &auswahl);
		printf("\n");
		...
		case 7:
			...
			break;
		default:
			printf("Bitte einen sinnvollen Wert "
				"zwischen 1 und 6 eingeben. \n");
		}
	}
	return 0;
}
